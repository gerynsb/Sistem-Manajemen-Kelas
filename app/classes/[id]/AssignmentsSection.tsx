        ...
        {assignments.length === 0 ? (
          ...
          assignments.map((assignment) => (
            <div key={assignment.id} className="p-4 bg-gray-100 rounded-lg mt-2">
              <p className="font-bold text-black">{assignment.title || "Judul Tidak Tersedia"}</p>
              <p className="text-sm text-gray-500">
                {assignment.dueDate && assignment.dueDate.seconds
                  ? new Date(assignment.dueDate.seconds * 1000).toLocaleDateString("id-ID", {
                      weekday: "long",
                      day: "2-digit",
                      month: "long",
                      year: "numeric",
